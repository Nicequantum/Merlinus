    ...
    <div className="benz-card benz-diagnostic-card p-5 min-w-0 w-full">
      ...
      {canProcess && !isProcessing && (
        <div className="flex gap-2 mb-3">
          ...
          >
            <Sparkles size={18} />
            {t('process', { count: imagesNeedingAnalysisCount })}
          </button>
          {hasPending && (
            <button
              type="button"
      ...
      {isProcessing && (
        <div className="benz-card p-4 mb-3 border border-benz-accent/20">
          <div className="flex items-center justify-between gap-2 mb-3">
            <div className="benz-section-title text-sm">{t('inProgress')}</div>
            <button
              type="button"
          ...
          <p className="text-xs text-benz-secondary flex items-center gap-2">
            ...
            {statusMessage || t('processing', { percent: ocrProgress })}
          </p>
        </div>
      )}
      ...
      {hasPending && (
        <div className="benz-card p-4 mb-3">
          <div className="benz-section-title mb-3 text-sm">
            {isProcessing
              ? t('processingCount', { count: pendingImages.length })
              : t('saving', { count: pendingImages.length })}
          </div>
          <DiagnosticPhotoGrid
      ...
      {hasSaved && (
        <>
          <div className="benz-section-title mb-2 text-sm">{t('title')}</div>
          <XentryImageGallery images={savedImages} onDeleteImage={onDeleteSavedImage} />
        </>
      )}

      <p className="benz-hint mt-3 leading-relaxed">{t('captureHint')}</p>
