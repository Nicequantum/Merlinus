...
export function LineView({
  ...
  const { t: tXentry } = useTranslation('xentry');
  ...
  const mileageStr = ro.vehicle.mileageIn ? `${ro.vehicle.mileageIn} ${t('mi')}` : '';
    ...
    <div className="benz-page pb-12">
      ...
      <div className="benz-line-flow">
        ...
        <XentryDiagnosticSection
          title={tXentry('diagnosticEvidence')}
        ...
        <div className="benz-generate-panel space-y-3 relative z-[5]">
          {isCustomerPayLine ? (
            <div className="benz-cp-instant-banner flex items-start gap-3 p-4 rounded-xl border border-benz-green/30 bg-benz-green/8">
              ...
              <div>
                <div className="text-sm font-semibold text-benz-primary">{t('cpBannerTitle')}</div>
                <p className="text-xs text-benz-secondary mt-1 leading-relaxed">{t('cpBannerBody')}</p>
              </div>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              {isGenerating && (
                <div
                  className="benz-gen-progress"
                  role="progressbar"
                  aria-valuenow={Math.round(generationPhase.progress)}
                  aria-valuemin={0}
                  aria-valuemax={100}
                >
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            ...
            {isCustomerPayLine && onClearCustomerPayMode && (
              <div className="benz-cp-switch-banner w-full">
                <p className="text-xs text-benz-secondary leading-relaxed">{t('switchToWarrantyHint')}</p>
                <button
                  type="button"
          ...
          <p className="benz-hint text-center">
            {isCustomerPayLine ? t('cpHint') : t('warrantyHint', { mi: MI_PRODUCT_LABEL })}
          </p>
        ...
        {!line.warrantyStory?.trim() && (
          <BenzEmptyState
            ...
            title={isCustomerPayLine ? t('emptyCpTitle') : t('emptyWarrantyTitle')}
            hint={isCustomerPayLine ? t('emptyCpHint') : t('emptyWarrantyHint')}
            actionLabel={isCustomerPayLine ? t('browseCpTemplates') : t('generateStory')}
                  ...
                  title={
                    certificationActionsLocked
                      ? storyComplianceState === 'not-audited'
                        ? t('copyTitleNeedAudit')
                        : storyComplianceState === 'audit-stale'
                          ? t('copyTitleNeedReaudit')
                          : t('copyTitleNeedCertify')
                      : undefined
                  }
