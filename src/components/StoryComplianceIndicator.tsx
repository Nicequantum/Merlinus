...
export function StoryComplianceIndicator({ state }: StoryComplianceIndicatorProps) {
  ...
  const statusLabel =
    state === 'not-audited'
      ? t('complianceNotAudited')
      : state === 'audited'
        ? t('complianceAudited')
        : state === 'audit-stale'
          ? t('complianceStale')
          : t('complianceCertified');

  const steps: { key: StoryComplianceState; label: string }[] = [
    { key: 'not-audited', label: t('complianceNotAudited') },
    { key: 'audited', label: t('complianceAudited') },
    { key: 'certified', label: t('complianceCertified') },
  ];
